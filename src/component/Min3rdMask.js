...
class Min3rdMask extends React.Component {
	...
	render() {
		return (
			<div>
				<p>
					Для узла с IP адресом {this.state.ip.join('.')} адрес сети равен {this.state.net.join('.')}. Чему равно наименьшее возможное значение третьего слева байта маски? Ответ запишите в виде
					десятичного числа.
				</p>
				...
				<div className={this.state.showAnswer ? '' : 'hide'}>
					...
					<p>
						<span className='bold'>Ответ:</span> {this.state.minMask[2]}
					</p>
				</div>
			</div>
		);
