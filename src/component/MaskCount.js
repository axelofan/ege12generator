...
class MaskCount extends React.Component {
	...
	render() {
		return (
			<div>
				<p>
					Для узла с IP адресом {this.state.ip.join('.')} адрес сети равен {this.state.net.join('.')}. Для скольких различных значений маски это возможно?
				</p>
				...
				<div className={this.state.showAnswer ? '' : 'hide'}>
					...
					<p>
						В маске может быть от {this.state.minCount} до {this.state.maxCount} единиц.
					</p>
					...
					<p>
						<span className='bold'>Ответ:</span> {this.state.maskCount}
					</p>
				</div>
			</div>
		);
