...
class App extends React.Component {
	...
	getRandomTask = () => {
		...
		if (this.state.maskCount > 1) {
			component.push(<MaskCount data={this.state} />, <Min1Mask data={this.state} />, <Max1Mask data={this.state} />, <Min0Mask data={this.state} />, <Max0Mask data={this.state} />);
			...
			if (2 ** (32 - this.state.maxCount) <= 65536) component.push(<MinAdress data={this.state} />);
			if (2 ** (32 - this.state.minCount) <= 65536) component.push(<MaxAdress data={this.state} />);
	...
	render() {
		return (
			<div>
				<p>
					<span className='number'>13</span> В терминологии сетей TCP/IP маской сети называется двоичное число, определяющее, какая часть IP-адреса узла сети относится к адресу сети, а какая – к
					адресу самого узла в этой сети. При этом в маске сначала (в старших разрядах) стоят единицы, а затем с некоторого места – нули. Обычно маска записывается по тем же правилам, что и IP-адрес,
					– в виде четырёх байтов, причём каждый байт записывается в виде десятичного числа. Адрес сети получается в результате применения поразрядной конъюнкции к заданному IP-адресу узла и маске.
