...
function generateMask(count1) {
	...
	for (let i = 0; i < 4; i++) {
		if (count1 > 8) result.push(byte[8]);
		else result.push(byte[count1]);
...
let taskData = { mask, net, ip, maskCount, minCount, maxCount, minMask: generateMask(minCount), maxMask: generateMask(maxCount) };
